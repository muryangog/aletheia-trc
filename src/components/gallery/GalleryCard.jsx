...
export default function GalleryCard({ photo, onClick }) {
  return (
    <div
      onClick={onClick}
      className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-150 group cursor-pointer">
      <div className="relative h-56 bg-slate-100 dark:bg-slate-800 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center z-10">
          <div className="p-3 bg-[#0c2448] rounded-lg text-white">
            <ZoomIn size={18} />
          </div>
        </div>
