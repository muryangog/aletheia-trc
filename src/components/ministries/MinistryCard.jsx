...
export default function MinistryCard({ dept, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-lg transition-all duration-200 flex flex-col justify-between group cursor-pointer">
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] font-bold text-[#48a848] bg-[#48a848]/10 px-2 py-0.5 rounded-xl uppercase tracking-wider">
            {dept.category}
          </span>
        </div>

        <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 mb-2 group-hover:text-[#48a848] transition-colors">
          {dept.name}
        </h3>
        ...
        <div className="bg-slate-50 dark:bg-slate-800/40 p-2.5 rounded-xl mb-4 border border-slate-100 dark:border-slate-800/60">
          <span className="text-[10px] font-bold text-slate-900 dark:text-slate-300 block mb-0.5">
            ⚙️ En pratique :
          </span>
